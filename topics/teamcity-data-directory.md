...
It is recommended to store `<`[`TeamCity Data Directory`](teamcity-data-directory.md)`>/system/caches` on a local disk and may be even a separate dedicated disk, especially when TeamCity Data Directory is lcoated on a network storage. The directory stores local clones of the VCS repositories and it is important to provide good performance for it. If the directory content is lost, it will be rebuilt and no data will be lost.
