...
If you perform a request from within a TeamCity build, for a limited set of build-related operations (like downloading artifacts) you can use values of [`teamcity.auth.userId/teamcity.auth.password`](artifact-dependencies.md#build-level-auth) system properties as basic credentials (within TeamCity settings you can reference them as `%\system.teamcity.auth.userId%` and `%\system.teamcity.auth.password%`).
 
Within a build, the request for current build details can look like:
 
```Shell
curl -u "%\system.teamcity.auth.userId%:%\system.teamcity.auth.password%" "%\teamcity.serverUrl%/httpAuth/app/rest/builds/id:%\teamcity.build.id%"
...
## CORS Support
{id="CORS-support" help-id="CORS Support"}
