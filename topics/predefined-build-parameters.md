...
Predefined build parameters can originate from several scopes:
...
There is also a special type of server-side build parameters that can be referenced by other parameters but that are not passed to a build. See the [list of such parameters](#Predefined+Configuration+Parameters).

>__Tips__:
>* System properties can be [referenced](using-build-parameters.md#Using+Build+Parameters+in+Build+Configuration+Settings) using `%\system.propertyName%`.
...
These parameters are unique for each build. They are calculated on the agent right before the build start and then passed to the build.
...
The [working directory](build-working-directory.md) where the build is started. This is the path where TeamCity build runner is supposed to start a process. This is a runner-specific property, thus it has a different value for each step.
...
A full name (including the path) of the file containing all the `system.*` properties passed to the build. The `system.` prefix is omitted. The file uses the [Java properties](http://java.sun.com/j2se/1.4.2/docs/api/java/util/Properties.html#load(java.io.InputStream)) file format (for example, special symbols are backslash-escaped).
...
These properties can be used for setting build configuration options, defining build configuration requirements (for example, check if a certain parameter exist), and inside build scripts. For more information on how to reference these properties, see [this section](configuring-build-parameters.md#Custom+Build+Parameters).
...
This parameter value is set to the corresponding framework runtime version(s) path(s).
...
Configuration parameters can be referenced by other parameters (only if defined on the __Parameters__ page), but they are not passed to the build themselves.

To get the full set of such server parameters, add the `system.teamcity.debug.dump.parameters` parameter [to a build configuration](configuring-build-parameters.md#Custom+Build+Parameters) and then view the "_Available server properties_" section in the build log.
...
In a [dependent build](dependent-build.md), dependency parameters have the following format:
...
It is possible to override values of [build parameters](configuring-build-parameters.md) of the snapshot-dependency builds in the current build, on its start.

For example, build configuration A depends on B and B depends on C. On triggering, A can change any parameter used in B or C with the following parameter:
...
If some dependency build configurations have alike IDs, that is their beginning and/or ending match, you can change a parameter in these dependencies by defining their prefix or/and suffix. Use the following syntax:
...
* `<VCS_root_ID>` — is the [VCS root ID](configuring-vcs-roots.md).
