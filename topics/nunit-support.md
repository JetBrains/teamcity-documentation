...
__Important notes__
* Be sure to replace `.` with `_` when [using system properties](configuring-build-parameters.md#Parameter+Types) in MSBuild scripts. For example, use `teamcity_dotnet_nunitlauncher_msbuild_task` instead of `teamcity.dotnet.nunitlauncher.msbuild.task`.
