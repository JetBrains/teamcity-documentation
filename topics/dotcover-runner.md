...
The **dotCover** runner uses the [JetBrains](https://www.jetbrains.com/dotcover/) to produce coverage reports for .NET processes. Generated reports are published to the [Code Coverage tab](build-results-page.md#Code+Coverage+Tab) of the Build Results page.
...
The **dotCover** runner can be used as a one-stop shop that tests the required project and generates a code coverage report (see the following [Kotlin DSL](kotlin-dsl.md) sample for an example setup).
...
Configuration B has its own [](net.md) that produces a snapshot. The final step of this configuration retrieves this snapshot, merges it with snapshots imported from Configuration A via [artifact dependencies](artifact-dependencies.md) and its own snapshot from testing "Project D", and publishes a joint report that reflects data for both build configurations.
