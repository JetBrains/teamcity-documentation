...
[Pipelines](create-and-edit-pipelines.md) aim to reimagine the familiar TeamCity experience to offer the most intuitive way to design complex CI/CD workflows. Their native configuration-as-code format is [YAML](pipelines-yaml-syntax.md).

Starting with version 2026.1, pipeline settings are automatically converted to Kotlin DSL if the parent project has its versioned settings enabled. See [](pipelines-dsl.md) for more information.
