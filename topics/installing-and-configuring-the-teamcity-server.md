...
To install a TeamCity server:

1. Choose the most convenient TeamCity [distribution option](#Installing+TeamCity+Server).
2. [Download](http://www.jetbrains.com/teamcity/download/) the distribution.
3. Review the notes on [software requirements](supported-platforms-and-environments.md), [hardware requirements](how-to.md#Estimate+Hardware+Requirements+for+TeamCity), and [platform selection](how-to.md#Choose+OS%2FPlatform+for+TeamCity+Server).
...
* [.war distribution](#Installing+TeamCity+into+Existing+J2EE+Container): for experienced users who want to run TeamCity on a separate Web application server.   
We do not recommend installing via `.war` unless it is extremely necessary (please [let us know](https://confluence.jetbrains.com/display/TW/Feedback) the reasons). Consider using the `.tar.gz` distribution instead.
...
* have the [teamcity-server startup script](#Starting+TeamCity+server) which provides several convenient options (for example, a separate environment variable for memory settings) and configures TeamCity correctly (for example, `log4j` configuration).
* (at least under Windows) provide better error reporting for some cases (like a missing Java installation).
* (under Windows) allow running TeamCity as a service with the ability to use the same configuration as if run from a console.
...
The build server and one build agent will be installed by default for Windows, Linux or macOS. If you need more build agents, refer to the [Installing Additional Build Agents](setting-up-and-running-additional-build-agents.md) page.
...
Otherwise, use standard [scripts](#Starting+TeamCity+server).
...
* all the necessary permissions to work with the source controls used. This includes:
   * access to Microsoft Visual SourceSafe database (if [Visual SourceSafe](visual-sourcesafe.md) integration is used)
...
3. Ensure the TeamCity web application gets sufficient amount of [memory](#Setting+Up+Memory+settings+for+TeamCity+Server). Increase the memory respectively if you have other web applications running in the same JVM.
...
If you are using __Tomcat__ J2EE container, make sure the [Apache Portable Runtime](http://tomcat.apache.org/tomcat-7.0-doc/apr.html) feature of this container is disabled (it is disabled by default). Otherwise, due to issues in the [Apache Portable Runtime](http://tomcat.apache.org/tomcat-7.0-doc/apr.html), TeamCity may not work properly.
</note>
...
If you want to get a preconfigured server right away, put files from a previously configured server into the Data Directory. For each new server you will need to ensure it points to a new database (configured in `<Data Directory>\config\database.properties`) and change `<Data Directory>\config\main-config.xml` file not to have the `uuid` attribute in the root XML element (so new one can be generated) and setting appropriate value for the `rootURL` attribute.
...
Upon successful installation, the TeamCity server web UI can be accessed via a web browser. The default address that can be used to access TeamCity from the same machine depends on the installation package and installation options. Port 80 is used for Windows installation, unless another port is specified; port 8111 is used for `.tar.gz` installation unless not changed in the server configuration.
...
To apply the changes, restart the server. If the server was previously working with the old port, you will need to change the port in all the stored URLs of the server (browser bookmarks, agents' serverUrl [property](build-agent-configuration.md), URL in user's IDEs, `Server URL` setting on the __Administration | Global Settings__ page).
...
If you want to use the `https://` protocol, it should be enabled separately, and this process is not specific to TeamCity, but rather for the web server used (Tomcat by default). See also [Configure HTTPS for TeamCity Web UI](how-to.md#Configure+HTTPS+for+TeamCity+Web+UI).
...
* __minimum setting__ (32-bit Java, bundled in `.exe` distribution, must be used): `-Xmx750m`
* __recommended setting__ for __medium__ server use (32\-bit Java must be used): `-Xmx1024m`. Greater settings with the 32-bit Java can cause `OutOfMemoryError` with "Native memory allocation (malloc) failed" JVM crashes or "unable to create new native thread" messages
* __recommended setting__ for __large__ server use ([64-bit Java](#Using+64+bit+Java+to+Run+TeamCity+Server) must be used): `-Xmx4g -XX:ReservedCodeCacheSize=350m`. These settings should be suitable for an installation with up to two hundreds of agents and thousands of build configurations. Custom plugins installed might require increasing the value defined via the Xmx parameter.
* __maximum settings__ for large-scale server use ([64-bit Java](#Using+64+bit+Java+to+Run+TeamCity+Server) must be used): `-Xmx10g -XX:ReservedCodeCacheSize=512m`. Greater values can be used for larger TeamCity installations. However, generally it is not recommended to use values greater than 10g without consulting TeamCity support.
...
* A rule of thumb is that the 64-bit JVM should be assigned twice as much memory as the 32\-bit for the same application. If you switch to the 64-bit JVM, make sure you adjust the memory setting (`-Xmx`) respectively. It does not make sense to switch to 64 bit if you dedicate less than the double amount of memory to the application.
...
After successful server start, any TeamCity page request will redirect to prompt for the server administrator username and password. Make sure no one can access the server pages until the administrator account is set up.
