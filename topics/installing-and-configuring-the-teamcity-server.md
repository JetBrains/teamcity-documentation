...
It is recommended to run TeamCity server with the latest Java 8 x64. The recommended Java distribution is [Amazon Coretto](https://aws.amazon.com/corretto/). Support for Java versions 9-11 is experimental, so Java 8 is still the recommended version.
...
* A rule of thumb is that the 64-bit JVM should be assigned twice as much memory as the 32-bit for the same application. If you switch to the 64-bit JVM (for example, on upgrading a bundled version to TeamCity 2020.1), make sure you adjust the memory setting (`-Xmx`) accordingly. It does not make sense to switch to 64-bit if you dedicate less than the double amount of memory to the application.
