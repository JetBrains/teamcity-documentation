...
* [Agent-side checkout](vcs-checkout-mode.md), the default mode. In this case TeamCity creates a Perforce workspace to checkout the sources
...
With [agent-side checkout](vcs-checkout-mode.md), TeamCity provides environment variables describing the Perforce workspace created during the checkout process.   
...
The variables are:
* `P4USER` \- same as `vcsroot.<VCS root ID>.user` [parameter](predefined-build-parameters.md)
* `P4PORT `\- same as `vcsroot.<VCS root ID>.port` [parameter](predefined-build-parameters.md)
...
When [agent-side checkout](vcs-checkout-mode.md) is used, the TeamCity Perforce plugin creates a workspace which is bound to the checkout directory on an agent. The checkout is performed with an incremental __p4 sync__ command (both for personal and non\-personal builds).
...
Usually, when a project administrator modifies a VCS Root client mapping specified in the VCS Root, this is considered as a change in VCS Root settings and results in a [clean checkout](clean-checkout.md). This clean checkout behaviour can be disabled using the `teamcity.perforce.enable-no-clean-checkout=true` [internal property](configuring-teamcity-server-startup-properties.md).

<note>

Changing `teamcity.perforce.enable-no-clean-checkout `[internal property](configuring-teamcity-server-startup-properties.md) results in one\-time clean checkout for all affected build configurations.
</note>
