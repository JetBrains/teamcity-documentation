...
During the startup, a TeamCity agent searches for the PowerShell installation in standard locations, such as `Program Files` and `Windows` directories, or `~/powershell` and `/opt/microsoft/powershell/<version>/` in ARM64 systems. You can specify a custom location in the `teamcity.powershell.detector.search.paths` [agent property](predefined-build-parameters.md#Agent+Parameters), so the agent can detect PowerShell in this directory (and its children) as well.   
