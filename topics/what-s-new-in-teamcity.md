...
Kotlin DSL code generated by previous TeamCity versions had imports with `v2019_2` in their names, for instance:
```Kotlin
...
But the presence of the several versions of packages caused constant troubles with code completion in IntelliJ IDEA, 
where it suggested several similar classes, with package names with different versions.
