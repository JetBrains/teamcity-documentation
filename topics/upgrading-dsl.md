...
you can continue using classes from the [v2017_2][jetbrains.buildServer.configs.kotlin.v2017_2] package even on TeamCity 2019.2. 
...
If there were any changes to projects configuration via the web interface while versioned settings were disabled, these changes will be committed to VCS repository too when settings are enabled. 
...
1) change all imports in DSL scripts to new DSL API package, for instance, imports like:
```kotlin
...
Usually this can be done with help of search and replace in all import statements. After that, and after the compilation errors which may arise after this change, are fixed, 
the modified Kotlin files can be committed and DSL should continue working as before.

2) change _configs version_ to the most recent one, for instance, if your DSL scripts were generated by TeamCity 2018.1, then your settings.kts has:

```kotlin
version = "2018.1"
```

To change it to _"2019.2"_ you should first review the health reports produced by your TeamCity server after it applied the most recent changes in your DSL scripts. 
These health reports describe what should be changed in your scripts so that no additional transformations were required and you could safely change _configs version_.   
...
Then no additional changes to your DSL are required and you can change the _configs version_ to _"2019.2"_.
In other case, you can see something like this:
...
You should review these suggestions and apply them to your DSL scripts. The _configs version_ can be changed only after all of these suggestions are applied.

<note>
Instead of doing manual upgrade procedure you can always re-generate all of your Kotlin DSL scripts for your projects, 
if you did not change them in VCS repository since versioned settings were enabled in your project. 

For this you can disable versioned settings, then enable them again and select "Overwrite..." option in the confirmation dialog.
After that TeamCity will made a commit to your VCS repository which will remove all previously generated Kotlin files and 
will add newly generated files corresponding to the current TeamCity version. 
</note>
...
If MSBuild or VS.Solution build steps have the parameter `toolsVersion` set to `MSBuildStep.MSBuildToolsVersion.V15_0`, the 
additional `version = MSBuildStep.MSBuildVersion.V15_0` parameter should be added:
