...
<!--
## Update DSL from 2024.07.x to 2024.12.x

Starting with version 2024.12, TeamCity uses the K2 Kotlin compiler. This change requires possible edits to your Kotlin DSL files. See the [K2 migration guide](https://kotlinlang.org/docs/k2-compiler-migration-guide.html) for more information.

Your project settings generated by TeamCity will likely remain unaffected. However, you might need to modify your custom `.kt/.kts` files. For example, if you have multiple separate script files that declare classes with identical names, it is now required to declare them into separate packages.

```Kotlin
// myScripts/file1.kts
package myScripts.mainScript // required by K2 compiler
...
// myScripts/file2.kts
package myScripts.secondaryScript // required by K2 compiler
