...
Your project settings generated by TeamCity will likely remain unaffected. However, you might need to modify your custom `.kt/.kts` files. For example, if you have multiple separate script files with that declare classes with identical name, it is now required to declare them into separate packages.
