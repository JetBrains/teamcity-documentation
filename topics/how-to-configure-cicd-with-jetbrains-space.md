...
Only system administrators can enable authentication via JetBrains Space in TeamCity. If you want to integrate Space repositories with TeamCity projects, having project administration rights is enough.
...
3. Configure _In-context Authorization_:
   ...
   3. (_Optional_) If you want TeamCity to be able to publish commit statuses to Space, you will need to add a respective permission.  
      Click __Configure__ and enable _Git Repositories | Report external check status_. If you are the selected project's administrator, you can approve this permission right away. Otherwise, wait until it is granted by the project administrator.
...
To be able to manually run builds on the Space repository sources, you need to [configure steps](configuring-build-steps.md) of the newly created build configuration. The simplest scenario is to add a [Command Line](command-line.md) step and define CLI commands to build your sources. Then, you can initiate a build based on the freshest sources of the default branch: click __Run__ in the upper right corner. TeamCity will launch a one-step build executing the defined CLI commands.  
To monitor changes in the Space repository and start builds on new commits __automatically__, you need to add a [VCS trigger](configuring-vcs-triggers.md).
...
Now, TeamCity will monitor merge requests submitted from the source branches of your repository. If a build is run on a merge request, TeamCity will (a) display the details of the request and its code review in the build results and (b) report the build status to the code review in Space.
