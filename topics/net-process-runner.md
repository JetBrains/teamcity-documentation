...
To run any .NET process in your build, it is recommended to use the [.NET runner](net.md) with the \<custom\> command instead. This runner is actively supported and provides more benefits comparing to the .NET Process runner (such as code coverage and ability to run a process inside a Docker/Podman container).
