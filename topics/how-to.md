...
* man\-in\-the middle concerns 
  ...
  * between a TeamCity agent and the TeamCity server: see [the section](setting-up-and-running-additional-build-agents.md).
...
TeamCity tries not to pass password values in the web UI (from a browser to the server) in clear text and uses RSA with 1024\-bit key to encrypt them. However, it is recommended to use the TeamCity web UI only via HTTPS so this precaution should not be relevant.TeamCity stores passwords in the settings (where the original password value is necessary to perform authentication in other systems) in a scrambled form. The scrambling is done using 3DES with a fixed key.
