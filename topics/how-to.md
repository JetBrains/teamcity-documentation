...
4. Copy `<[TeamCity Data Directory](teamcity-data-directory.md)>`. If you do not need the full copy, refer to the items below for options.
    ...
    * `<TeamCity Data Directory>/system/caches` and `<TeamCity Data Directory>/system/caches_main` (optional) do not need to be copied to the new server. If not copied, they will be rebuilt on startup, and it may take some time.
...
>If you want to do a quick check and do not need to preserve the build history, users, and roles on the new server, you can skip Step 6 (cloning database) and all the optional items of Step 4.
