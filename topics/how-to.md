...
4. Copy `<[TeamCity Data Directory](teamcity-data-directory.md)>`. If you do not need the full copy, refer to the items below for options.
    ...
    * `<TeamCity Data Directory>/system/caches` and `<TeamCity Data Directory>/system/caches_main` (optional) are not necessary to copy to the new server. If not copied, they will be rebuilt on startup, but that can take some time (expect some slow down)
