...
* __Details__:
  ...
  * CPU rank: the result of the bundled CPU benchmark. Note that the benchmark results can depend on the JVM version and options used by the agent. For example, `-server` JVM option has a significant influence on the results. CPU benchmark also affects the way the server distributes builds among agents. If TeamCity cannot calculate agent-specific [build duration estimates](working-with-build-queue.md#build-duration-estimates), or if the estimates are the same for all compatible agents, TeamCity chooses the fastest agent based on the CPU benchmark value.
