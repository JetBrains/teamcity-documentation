...
* _Build parameters_: customize the value of any [parameter](configuring-build-parameters.md) used in the current build configuration. Or, add a new parameter, and it will be available only in builds started by this trigger. You can even use it to override a parameter of a [dependency build configuration](configuring-dependencies.md): use the [`reverse.dep.<dependencyBuildID>.<property>`](use-parameters-in-build-chains.md#Override+parameters+of+upstream+objects) syntax for this.
