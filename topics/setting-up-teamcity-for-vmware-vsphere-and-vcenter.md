...
* Select the type of behavior:
  * Start/stop an existing virtual machine, or
  * Clone a virtual machine or a template and deleting the clone once it becomes idle (when the build is finished or an idle timeout elapses, depending on the profile settings).
...
When configuring a TeamCity build agent, be sure to specify the valid TeamCity server URL in the build agent properties.

Notes on configuring a cloud profile:
