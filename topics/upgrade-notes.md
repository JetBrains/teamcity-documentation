...
## Changes from 2025.11.8 to 2026.1
{id="2026.1"}
...
* In previous versions, the `latest-linux-sudo` Docker image tag used the AMD64 manifest. Starting from version 2026.1, this tag uses a multi-architecture manifest combining AMD64 and ARM64 images. Platform-specific images are available under the `{version}-linux-amd64-sudo` and `{version}-linux-arm64-sudo` tags.
...
### Bundled Tools Updates
{id="bundled-tools-updates-2025-11-3"}

* The bundled Git was updated to version 2.53 in both server and agent Docker images.
* Docker shipped in TeamCity images now has a fixed GID of **999** to ensure consistency across releases for users who may want to reference it directly in their build scripts.
...
* If agents spawned via a K8S cloud profile with Windows pods need to upgrade, agent K8S pod stop and a replacement pod spawns. This issue is addressed in version 2026.1. For previous versions, use a workaround provided in the following ticket: [TW-93396](https://youtrack.jetbrains.com/issue/TW-93396).
...
* Build configurations created via the [updated flow](creating-and-editing-build-configurations.md#Create+Build+Configurations+in+TeamCity+UI) no longer include the [](performance-monitor.md) build feature by default. Add this feature manually if you wish to track the agent CPU, disk, and memory usage during builds.
    ...
    Related YouTrack ticket: [TW-97487](https://youtrack.jetbrains.com/issue/TW-97487/).

* Build configurations created via the [updated flow](creating-and-editing-build-configurations.md#Create+Build+Configurations+in+TeamCity+UI) utilize the "Do not use mirrors" [checkout policy](git.md#git-checkout-policy), as opposed to the "Auto" policy used in the classic UI.

    Related YouTrack ticket: [TW-97576](https://youtrack.jetbrains.com/issue/TW-97576).

* The **Import from DSL (classic UI)** link on the updated new project page does not open the expected page if the project name field is blank.

    Related YouTrack ticket: [TW-97529](https://youtrack.jetbrains.com/issue/TW-97529).
...
* __C# Script builds may fail if .NET profiling is enabled__  
  ...
  To work around it, add an [environment variable](configuring-build-parameters.md#Parameter+Types) `COMPlus_EnableDiagnostics=0` to your build configuration.
...
* __Builds might fail to publish artifacts to Amazon S3 if AWS KMS is used__  
  After updating to 2021.2, builds might start failing on an attempt to publish artifacts to an Amazon S3 bucket encrypted with an [AWS KMS](https://docs.aws.amazon.com/kms/index.html) key. This issue is caused by the recently added integrity check for build artifacts. To temporarily disable it in a project and workaround the issue, set the `teamcity.internal.storage.s3.upload.enableConsistencyCheck=false` property on a [project level](configuring-build-parameters.md#Parameter+Sources).  
