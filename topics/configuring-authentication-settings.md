...
You can upload an extra certificate in case you are about to change one on the SSO provider's side. Having a new certificate preuploaded in TeamCity will ensure uninterrupted trusted connection with the provider during and after the upgrade.
...
After you save the SAML settings for the first time, TeamCity exports them to the SP metadata XML file. This file is located via the `https://<yourHostName>.teamcity.com/app/saml/metadata.xml` path and can be downloaded in this settings section. You can upload this file on your SSO provider's side to apply the TeamCity configuration automatically.
