...
The module relies on the verified third-party [SAML Authentication plugin](https://github.com/morincer/teamcity-plugin-saml). To get the plugin, open the latest successful build of the [TeamCity Plugin SAML](https://teamcity.jetbrains.com/buildConfiguration/TeamCityPluginsByJetBrains_TeamcityPluginSaml_Build) project on `teamcity.jetbrains.com`, and download `saml-authentication.zip` from the build artifacts.
{instance="tc"}
