...
Before enabling this module, you need to create a dedicated application in JetBrains Space and configure a connection to it in the Root project's settings.

In JetBrains Space:
1. Go to __Administration | Applications__ and click __New application__.
2. Enter a convenient name (for example, _TeamCity Auth_).
3. In the __Requested rights__ tab, enable the _View member profile_ permission.
4. In the __Authorization__ tab, click __Add project__ and enter the names of the source projects in which you want to authorize this app.
5. In the __Authentication__ tab, enable _Authorization Code Flow_ and enter your TeamCity server's URL as the redirect URI.  
   ...
   >There is a [known issue](https://youtrack.jetbrains.com/issue/TW-72441) regarding the URIs necessary for connection. If you fail to authenticate in Space, you will see extra URIs that have to be added in this field as well. This issue will be resolved in the 2021.2 EAP2.
   > 
   {type="warning"}
6. Copy the app's autogenerated _Client ID_ and _Client secret_.
...
* In __Administation | Authentication__:
