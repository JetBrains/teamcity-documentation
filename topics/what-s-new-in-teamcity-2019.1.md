...
[//]: # (help-id: What's New in TeamCity 2019.1)
...
* If the '_Docker Registry Connections_' feature is added to a build configuration, the TeamCity server will automatically add an agent requirement for an installed Docker, so the builds will run on compatible agents only. 
